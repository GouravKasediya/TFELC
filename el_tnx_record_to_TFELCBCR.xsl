...
<xsl:stylesheet version="2.0"
  ...
  exclude-result-prefixes="exsl core params">
  ...
  <xsl:template match="el_tnx_record">
    ...
    <ServiceRequest xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance'
      ...
      <xsl:call-template name="RequestHeader">
        <xsl:with-param name="operation">TFELCBCR</xsl:with-param>
      ...
      <m:TFELCBCR>
        ...
        <m:CancelRef><xsl:value-of select="bo_tnx_id" /></m:CancelRef>
        <m:Sender>
        ...
        <m:ResponseType><xsl:value-of select="if (sub_tnx_type_code = '46') then 'A'
                                              else if (sub_tnx_type_code = '47') then 'R'
                                              else ''" /></m:ResponseType>
  ...
  <!--  <xsl:include href="../custom/incoming/el_tnx_record_to_TFELCBCR_custom.xsl" />  -->
